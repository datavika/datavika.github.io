...
function renderUday(){
  ...
  if(ub) ub.innerHTML=`
    <div style="background:linear-gradient(135deg,rgba(245,166,35,.07),rgba(245,166,35,.02));border:1px solid rgba(245,166,35,.22);border-radius:var(--r-xl);padding:26px 30px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:18px">
      <div style="flex:1;min-width:240px">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
          <span style="font-size:.66rem;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--amber)">🌅 DataVika Uday Batch</span>
          <span style="font-size:.6rem;font-weight:700;color:#060D06;background:var(--amber);padding:2px 8px;border-radius:50px">Coming Soon</span>
        </div>
        <h3 style="font-size:1rem;margin-bottom:5px">Scholarship Program — 50% fee waiver for eligible students</h3>
        <p style="font-size:.82rem;color:var(--t2);margin-bottom:8px;max-width:440px">Financial background should never stop a motivated student. Apply to check eligibility.</p>
        <div style="display:flex;flex-direction:column;gap:3px">
          ${DV.uday.rules.slice(0,3).map(s=>`<div style="font-size:.74rem;color:var(--t2);display:flex;gap:6px"><span style="color:var(--amber);flex-shrink:0">→</span>${s}</div>`).join('')}
        </div>
      </div>
      <div style="display:flex;flex-direction:column;gap:9px;align-items:flex-end">
        <div style="font-size:.7rem;color:rgba(245,166,35,.5);text-align:right;max-width:180px">Join community to be notified when applications open</div>
        <a href="https://chat.whatsapp.com/Jvf40wFUenE70ZaHDH6Gwb" target="_blank" class="btn btn-a">Join Community →</a>
      </div>
    </div>`;
  const ur=document.getElementById('uday-rules');
  if(ur) ur.innerHTML=DV.uday.rules.map(r=>`<div class="uday-rule"><span class="uday-rule-ico">→</span><span>${r}</span></div>`).join('');
  const ui=document.getElementById('uday-includes');
  if(ui) ui.innerHTML=DV.uday.includes.map(i=>`<div class="uday-inc"><div class="uday-inc-dot"></div><span>${i}</span></div>`).join('');
}

/* ── CONSULTING ── */
function renderConsulting(){
  if(typeof DV==='undefined') return;
  const html=DV.consulting.map(s=>`<div class="card card-action" style="padding:18px"><div style="font-size:1.3rem;margin-bottom:8px">${s.icon}</div><div style="font-family:var(--font-d);font-size:.88rem;font-weight:700;color:var(--t1);margin-bottom:5px">${s.title}</div><div style="font-size:.78rem;color:var(--t2);margin-bottom:10px">${s.desc}</div><div style="font-family:var(--font-d);font-size:.98rem;font-weight:800;color:var(--amber)">${s.fee}</div></div>`).join('');
  const g1=document.getElementById('consult-grid');if(g1)g1.innerHTML=html;
  const g2=document.getElementById('fee-consulting');if(g2)g2.innerHTML=html;
}
...
function renderFooterNav(){
  ...
  if(!el)return;
  el.innerHTML=[{h:'#hero',t:'Home'},{h:'#about',t:'About'},{h:'#courses',t:'Courses'},{h:'#syllabus',t:'Syllabus'},{h:'#packs',t:'Special Packs'},{h:'#pricing',t:'Consulting'},{h:'#uday',t:'Uday Batch'},{h:'#resources',t:'Resources'},{h:'#events',t:'Events'},{h:'#consulting',t:'Consulting'},{h:'#contact',t:'Book Free Demo'}].map(l=>`<li><a href="${l.h}">${l.t}</a></li>`).join('');
}
